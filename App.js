...
// ✅ Import config unifié (une seule façon)
...
function EcranConnexion({ navigation, apiUrl }) {
  ...
  const handleConnexion = async () => {
    ...
    try {
      ...
      if (role === 'agent_sante')
        navigation.replace('DashboardAgent',       { user: data.user, token: data.token });
      else if (role === 'responsable_pev' || role === 'direction_departementale')
        navigation.replace('DashboardResponsable', { user: data.user, token: data.token });
      else if (role === 'mere' || role === 'pere')
        navigation.replace('DashboardParent',      { user: data.user, token: data.token });
      else if (role === 'relais')
        navigation.replace('DashboardRelais',      { user: data.user, token: data.token });
      else
